...
const Murl2symb = require('./murl2symb.js');
        ...
        if (!(/^\d+$/).test(message)) {
            if (message.substr(0, 1) === 'a') {
                ...
                if (messageEx.message.startsWith('pid-')) {
                    ...
                    Mpg.Save2pg('irealtime', [pid, last, bid, ask, high, low, last_close, pc, pcp, turnover, turnover_numeric, time, timestamp, date])
                        .then(res => {})
                        .catch(err => {
                    ...
                    Mpg.Save2pg('iexthours', [pid, last, bid, ask, pc, pcp, turnover, turnover_numeric, time, timestamp, date])
                        .then(res => {})
                        .catch(err => {
                            console.error(err.stack);
                        });
...
refreshList = function(listname) {
    ...
    const color = listname === 'top-stock-gainers?country=usa' ? 'green' : 'red';
...
function Saveiqmap(pid, url) {
    // setups drived purely by idata => gdata together with idata
    Mpg.getSymbByPid(pid)
        .then(function(symb) {
            ...
            if (symb === undefined) {
                ...
                Murl2symb.getSymbByUrl(url)
                    .then(function(symb) {
                        //console.log('then symb: ' + symb);
                        //'' or valid symb
                        if (symb === '') {
                            console.log(`no symb found by ${url} for ${pid}`);

                            Mheartbeat.sendInfo1(pid, url);

                            Mpg.Save2pg('iqmap', [pid, url])
                                .catch(err => {
                                    console.error(err.stack);
                                });
                            ...
                            Mpg.Save2pg('iqmap', [pid, url, symb])
                                .catch(err => {
                                    console.error(err.stack);
                                });
                        }
...
var job1 = schedule.scheduleJob('3 ' + minute.toString() + ' ' + hour.toString() + ' ' + day.toString() + ' ' + month.toString() + ' *', function() {
    ...
    Mpg.getSelectedPidArray().then((res) => {
            pidArray = _.union(pidArray, res);
            // load position
...
schedule.scheduleJob('13 */5 * * * *', function() {
...
// exclude list: SetupID, Pid, Symb
function checkSetups(pid, symb) {
    ...
    */
    ...
    checkISetup_No1BoxBreakout(pid, symb); // use g to check box and i to check breakout

    checkGSetup_No2TopLCrossUpMAs(pid, symb); // use g to check box and i to check MAs-cross-up
}

// include list: SetupID, Pid, Symb, Qid
function checkPositionSell(symb) {
